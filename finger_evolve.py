...
def run_generation(population, gen_id):
    ...
    for i in range(len(population)):
        ...
        file = open("losses.pkl", "rb")
        data = pickle.load(file)
        file.close()
        pop_losses.append(data)

    all_losses.append(pop_losses)

    iteration_losses_min.append(min(losses))
    pop_best.append(iteration_losses_min.index(min(iteration_losses_min)))
    iteration_losses_avg.append(sum(losses)/len(losses))
    return losses

def purge_replace_population(population, losses, p_keep=0.4):
    new_population = []
    min_loss = min(losses)
    purged = 0
...
def main():
    ...
    final_losses = [pop_best, iteration_losses_min, iteration_losses_avg, all_losses]
    file = open('final_losses.pkl', 'wb')
