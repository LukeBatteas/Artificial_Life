...
def nn1(t: ti.i32):
    for i in range(n_hidden):
        ...
        for j in ti.static(range(n_sin_waves)):
            # actuation += weights1[i, j] * step(t, 1000 * (1 - (j + 1) / n_sin_waves))
        ...
        actuation += weights1[i, n_objects * 6 + n_sin_waves] * x[t, 0][0]
        actuation += weights1[i, n_objects * 6 + n_sin_waves + 1] * x[t, 0][1]
        actuation += bias1[i]
...
def compute_loss(t: ti.i32):
    loss[None] = (x[t, tip_id_2] - x[t, tip_id_1]).norm()
...
def setup_robot(objects, springs, tip_id1, tip_id2):
    global tip_id_1
    tip_id_1 = tip_id1
    global tip_id_2
    tip_id_2 = tip_id2
...
# palm = [0.3, 0.4, 0.02, 0.12]
# phalange_1 = [0.05, 0.03, 10]
# phalange_2 = [0.04, 0.025, 10]
# phalange_3 = [0.05, 0.03, 10]
# phalanges = [phalange_1, phalange_2, phalange_3]
# big_phalanx = [0.1, 0.06, 10]


def run_body(individual, gen_id=0, idp=0):

    setup_robot(*hand(individual[0], individual[1], individual[2], individual[3]))

    losses = optimize(toi=True, visualize=False)

    clear_states()
    forward('fingers/gen' + str(gen_id) + '/pop'+ str(idp))
    # ti.root.deactivate_all()  # Clears the root, which contains all fields and variables
    ti.reset()
    file = open('losses.pkl', 'wb')
...
#     setup_robot(*hand(palm, phalanges, None))

#     losses = optimize(toi=True, visualize=visualize)
...
if __name__ == '__main__':
    ...
    file.close()
    run_body(data, int(sys.argv[1]), int(sys.argv[2]))
